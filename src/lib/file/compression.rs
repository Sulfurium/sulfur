...
pub async fn compression<P: Clone + AsRef<Path>>(folder: P, file: P) {
    let created_file = std::fs::File::create(file.as_ref()).unwrap();

    let folder_file = get_list_of_file(folder.as_ref().to_str().unwrap().to_string());
...
pub fn get_list_of_file(folder: String) -> Vec<String> {
    let read_dir = std::fs::read_dir(folder).unwrap();
    let mut result = Vec::new();
    for res in read_dir {
        let entry = res.unwrap();
        if entry.metadata().unwrap().is_dir() {
            result.append(&mut get_list_of_file(entry.path().to_str().unwrap().to_string()));
        } else {
